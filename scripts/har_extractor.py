...
def response(context, flow):
    ...
    # that format.
    timings = dict([(key, int(1000 * value)) for key, value in timings_raw.items()])
    ...
    request_cookies = [{"name": k.strip(), "value": v[0]} for k, v in (flow.request.get_cookies() or {}).items()]
    ...
    # Cookies are shaped as tuples by MITMProxy.
    response_cookies = [{"name": k.strip(), "value": v[0]} for k, v in (flow.response.get_cookies() or {}).items()]
...
def done(context):
    ...
    json_dump = context.HARLog.json(log=True)
